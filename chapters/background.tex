...
When computing the upper $N/2$ terms, i.e. when $N/2 \leq k < N$, $X_k = E_{k-N/2} + T^{N/2}O_{k-N/2}$, which are the same O and E as the lower $N/2$ terms, just with an offset of N/2. The the twiddle factor is raised to the power of N/2 because it too was in terms of k. We can thus split $X_k$ into \todo{This section could be reworked}

$$X_k = E_k + TO_k$$  
$$X_{k+N/2} = E_k + T^{N/2}O_k$$

which are the equations for the lower and upper $N/2$ terms. Finally the twiddle factor is rewritten using the following property 
...
This means that when the element indeces are adjusted, $E_k$ and $O_k$ are both DFTs essentially. To transform (no pun intended) this purely mathematical notation into something that looks more like a complete algorithm,factor out $E_k$ and $O_k$, express the computations with a loop
...
\end{lstlisting}
Fortran indexes starting at 1, which means the upper loop boundary nicely becomes just $N/2$, but the k in the exponent needs to be adjusted. Before these computations we need to compute the evens and odds.
\begin{lstlisting}
    E = r2fft(input(1:N:2))
    O = r2fft(input(2:N:2))
\end{lstlisting}
A recursive algorithm also needs a base case
\begin{lstlisting}    
if (N == 1) then
    X = input
    return
...
\end{lstlisting}
With the final programmatic things, a naive Radix-2 FFT could look like the following
\begin{lstlisting}
