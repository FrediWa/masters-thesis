...
% but why a matrix when the summation formula is both practical and more compact..? Also, FFT? What's the damn point.
...
% The "periodicity of E and O are still very weird to me
% Source of proof: https://www.phys.uconn.edu/~rozman/Courses/m3511_19s/downloads/radix2fft.pdf

Once the algorithm is derived, turning it into code is as straight forward as the DFT. Figure \ref{fig:FFT-Alg} shows how the process evolves for an input vector of length 8.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{./images/fft.png}
    \caption{Hand drawn graph of an FFT. At any stage, evens are colored in blue and odds in orange to illustrate how the values are combined. "Ret" stands for return, to indicate that the current recursion iteration has ended. "C" stands for combine and y is just the partial result. The last vector Y is the output of the FFT.\label{fig:FFT-Alg}}
\end{figure}

\subsubsection{Inverse FFT}
The inverse of the FFT can be derived using the DFT matrix previously shown. It can be shown that inverting the DFT matrix results in a very similar matrix, but with all exponents of the twiddle factor negated and with a normalization scaling. In \cite{Reducible2020} it is shown that the IFFT algorithm can reuse the FFT logic, with a single line changed. At the time of writing, popular Python library Numpy utilizes the same \_raw\_fft method to compute both FFT and IFFT. %does this need source? https://github.com/numpy/numpy/blob/main/numpy/fft/_pocketfft.py
